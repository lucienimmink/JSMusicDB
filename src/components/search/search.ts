...
export class SearchNav extends LitElement {
  ...
  _handleTrackClick (e:Event, track: any) {
    e.preventDefault();
    this.navigate(`/letter/${track.album.artist.letter.escapedLetter}/artist/${track.album.artist.escapedName}/album/${track.album.escapedName}`);
  }
  navigate(href: any) {
  ...
  render() {
    return html`
      ${this.artists?.list.length > 0 ? html`
        <div class="container">
          ${this.artists?.overflow ? html`
            <div class="warn">Your search yielded over <strong>${MAX}</strong> results Only the first ${MAX} are shown. Please try to refine your search query</div>
          ` : nothing }
          <ol>
            <li class="header">Artists <span class="small muted">(${this.artists?.list.length})</span></li>
            ${this.artists?.list.map((artist: any) => html`
            <li>
      ...
      ` : nothing }
      ${this.albums?.list.length > 0 ? html`
      <div class="container">
        ${this.albums?.overflow ? html`
          <div class="warn">Your search yielded over <strong>${MAX}</strong> results Only the first ${MAX} are shown. Please try to refine your search query</div>
        ` : nothing }
        <ol>
          <li class="header">Albums <span class="small muted">(${this.albums?.list.length})</span></li>
          ${this.albums?.list.map((album: any) => html`
          <li>
      ...
      ` : nothing }
      ${this.tracks?.list.length > 0 ? html`
      <div class="container">
        ${this.tracks?.overflow ? html`
          <div class="warn">Your search yielded over <strong>${MAX}</strong> results Only the first ${MAX} are shown. Please try to refine your search query</div>
        ` : nothing }
        <ol>
          <li class="header">Tracks <span class="small muted">(${this.tracks?.list.length})</span></li>
          ${this.tracks?.list.map((track: any) => html`
            <li class="track" @click=${(e: Event) => this._handleTrackClick(e, track)}>
              <span class="title">
                ${track.title} <br />
                  <span class="small muted">${track.trackArtist} &bull; ${track.album.name}</span>
              </span>
              <span class="time">${timeSpan(track.duration)} <br />
                ${track.position > 0 && (track.isPlaying || track.isPaused) ? html`
                  <span class="small muted">${timeSpan(track.position)}</span>
                ` : html`
