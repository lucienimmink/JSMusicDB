...
export const getSimilairArtists = (artist: any) => {
  ...
  return fetch(`https://ws.audioscrobbler.com/2.0/?${params.toString()}`).then(response => response.json());
}
...
export const getTrackInfo = (track: any, user:string) => {
  ...
  params.set("artist", track.album.artist.albumArtist || track.album.artist.name)
...
export const toggleLoved = async (track:any, isLoved = true) => {
  ...
  params.set("api_key", APIKEY)
  params.set("api_sig", _signTrack(track.album.artist.albumArtist || track.album.artist.name, track.album.name, track.title, null, sk, method))
  params.set("artist", track.album.artist.albumArtist || track.album.artist.name)
  params.set("album", track.album.name)
  params.set("track", track.title)
  params.set("sk", sk);
...
export const authenticate = async ({username, password}:{username: string; password: string}) => {
  ...
  return fetch(`https://ws.audioscrobbler.com/2.0/?method=auth.getMobileSession`, {
    method: 'post',
    headers: {
      "content-type": "application/x-www-form-urlencoded"
    },
...
export const announceNowPlaying = async (track: any) => {
  ...
  const now = new Date();
  const timestamp = Date.UTC(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours(), now.getMinutes() + now.getTimezoneOffset(), now.getSeconds()) / 1000;
  const sk: string = await get(SK);
  if (!sk) {
    throw Error('No sign key');
  }
  ...
  params.set("api_key", APIKEY)
  params.set("api_sig", _signTrack(track.trackArtist, track.album.name, track.title, timestamp, sk, "track.updateNowPlaying"))
...
export const scrobbleTrack = async (track: any) => {
  ...
  params.set("api_key", APIKEY)
  params.set("api_sig", _signTrack(track.trackArtist, track.album.name, track.title, timestamp, sk, "track.scrobble"))
  params.set("artist", track.trackArtist)
  params.set("album", track.album.name)
  params.set("track", track.title)
  params.set("timestamp", timestamp.toString())
...
const _signAuthentication = (user: string, password: string):string => {
  return encryption.hex_md5(`api_key${APIKEY}methodauth.getMobileSessionpassword${password}username${user}${SECRET}`);
}

const _signTrack = (artist: string, album: string, track: string, timestamp: any, sk: string, method: string): string => {
  if (timestamp) {
    return encryption.hex_md5(`album${album}api_key${APIKEY}artist${artist}method${method}sk${sk}timestamp${timestamp}track${track}${SECRET}`);
  } else {
    return encryption.hex_md5(`album${album}api_key${APIKEY}artist${artist}method${method}sk${sk}track${track}${SECRET}`);
  }
}
