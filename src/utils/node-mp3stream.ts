...
const _semver = (v1: string, v2: string) => {
  ...
  if (Number(v1s[0]) === Number(v2s[0]) && Number(v1s[1]) > Number(v2s[1])) return true; // minor
  if (Number(v1s[0]) === Number(v2s[0]) && Number(v1s[1]) === Number(v2s[1]) && Number(v1s[2]) >= Number(v2s[2])) return true; // minor
  return false;
}
