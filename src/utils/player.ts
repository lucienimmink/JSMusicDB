...
export const startPlaylist = () => {
  document.querySelector("lit-musicdb")?.dispatchEvent(new CustomEvent(START_CURRENT_PLAYLIST));
}
export const getNextPlaylist = (playlist: any) => {
  switch (playlist.type) {
    ...
    default:
      throw new Error(`No implementation to generate a new playlist for type ${playlist.type}`)
  }
}
...
export const getNewPlaylistForRandom = (playlist: any) => {
  return new Promise((resolve, reject) => {
    musicdb.then((mdb:any) => {
      ...
      const randomTracks: string[] = _shuffle(trackIDs).splice(0, playlist.max);
      const nextPlaylist = {
...
export const getNewPlaylistForRandomPref = (playlist: any) => {
  return new Promise((resolve, reject) => {
    return musicdb.then((mdb:any) => {
      ...
      getTopArtists(playlist.username).then(({topartists}: { topartists: any }) => {
        ...
        for (let i = 0; i < playlist.max; i++) {
          if (i % 3 === 0 || i % 5 === 0) {
            ...
            // @ts-ignore
            newPlaylist.tracks.push(_getRandomTrackFromArtists(mediumRotation));
          } else {
            // @ts-ignore
            newPlaylist.tracks.push(_getRandomTrackFromArtists(mdb.artistsList()));
          }
        }
...
export const getNewPlaylistForLovedTracks = (playlist: any) => {
  return new Promise((resolve, reject) => {
    return musicdb.then((mdb:any) => {
      getLovedTracks(playlist.username).then(({lovedtracks }:{lovedtracks: any}) => {
        const newPlaylist = {
...
const _getNewPlaylistForAlbum = (playlist: any) => {
  return new Promise((resolve, reject) => {
    return musicdb.then((mdb:any) => {
      ...
      const nextPlaylist = {
        name: `${nextAlbum.artist.albumArtist || nextAlbum.artist.name} • ${nextAlbum.name}`,
...
const _getNextTrack = (artist: any, mdb: any, playlist: any) => {
  const promise = new Promise((resolve, reject) => {
    _getNextSimilarArtist(artist, mdb).then((similarartists: any) => {
